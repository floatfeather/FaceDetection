...
void draw(const Mat& img, const vector<int>& face) {
    if (face.size() != 4) {
        imshow( "result", img );
        return;
    }
...
int main( int argc, const char** argv )
{
    ...
    if(capture.isOpened())
    {
        ...
        for(;;)
        {
            ...
            vector<int> face;
            if (!face_detection::opencv_vj_detect(frame1, &face)) {
        ...
        vector<int> face;
        if (!face_detection::opencv_vj_detect(image, &face)) {
            cerr << "Error encountered in face detection" << endl;
            return 1;
        }
        draw(image, face);
